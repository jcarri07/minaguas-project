...
$semanas = calcularSemanas($fecha1, $fecha2);
$j = 0;
$r = mysqli_query($conn, "SELECT DAYOFWEEK(fecha) AS dia,WEEK(fecha) semana , MAX(CONCAT(fecha, ' ', hora)) AS fecha_hora, d.id_embalse
...
$res = mysqli_query($conn, "SELECT DAYOFWEEK(fecha) AS dia,WEEK(fecha) semana , MAX(CONCAT(fecha, ' ', hora)) AS fecha_hora, d.id_embalse
FROM  datos_embalse d
RIGHT JOIN embalses e ON e.id_embalse = d.id_embalse AND e.id_embalse = '$id'
WHERE fecha BETWEEN '$fecha1' AND '$fecha2' AND d.estatus = 'activo' AND (DAYOFWEEK(fecha) = 1 OR ( fecha = '$fecha2' AND DAYOFWEEK('$fecha2') != 1))
GROUP BY fecha;");

$datos1 = mysqli_fetch_all($res, MYSQLI_ASSOC);
$datos2 = mysqli_fetch_all($r, MYSQLI_ASSOC);
...
while ($semanas > 0) {
    ...
    if (isset($datos1[$j]['semana'])) {
        if ($numeroSemana == $datos1[$j]['semana']) {
            echo 'lunes:' . $datos1[$j]['fecha'] . '<br>';
            $numeroSemana++;
            $semanas--;
        } else {
            echo 'lunes:vacio<br>';
